/**
 * events.html sayfasındaki kartları Supabase'teki verilerden üretir.
...
(function () {
  ...
  function cardHTML(evt) {
    ...
    const ctaHref = evt.link && evt.link !== "" ? evt.link : "/form/";
  ...
  async function render() {
    const all = await VbaEvents.getAll();
    ...
    grid.querySelectorAll(".vba-del-evt").forEach((btn) => {
      btn.addEventListener("click", async () => {
        if (confirm("Bu etkinliği silmek istediğinize emin misiniz?")) {
          await VbaEvents.remove(btn.dataset.id);
          render();
        }
      });
    });
    grid.querySelectorAll(".vba-edit-evt").forEach((btn) => {
      btn.addEventListener("click", () => {
        window.location.href = "/yonetim/#etkinlik-" + btn.dataset.id;
      });
    });
  }
  ...
  // Giriş/çıkış durumu değişince (ör. admin.html'de login olunca) kartları yenile
