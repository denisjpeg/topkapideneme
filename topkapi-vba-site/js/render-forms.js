/**
 * forms.html sayfasındaki form kartlarını js/store.js'teki verilerden üretir.
...
(function () {
  ...
  function render() {
    ...
    grid.querySelectorAll(".vba-del-frm").forEach((btn) => {
      btn.addEventListener("click", () => {
        if (confirm("Bu formu silmek istediğinize emin misiniz?")) {
          VbaForms.remove(btn.dataset.id);
          render();
        }
      });
    });
    grid.querySelectorAll(".vba-edit-frm").forEach((btn) => {
      btn.addEventListener("click", () => {
        window.location.href = "admin.html#form-" + btn.dataset.id;
      });
    });
  }
